...
<body>
	...
	<div id="wrap" class="mypage">
						...
						<table>
							...
							<tbody>
								<c:forEach items="${autionList}" var="auction">
									...
									<tr>
										<td><fmt:formatDate value='${auction.tenderRegdate}'
												pattern='yyyy-MM-dd' /></td>
										<td>${auction.memName}</td>
										<td><c:choose>
												<c:when
													test="${auction.tenderStatus=='ing' && auction.dday<0}">
													<span class="redBack textstyle">유찰</span>
												</c:when>
												<c:when test="${auction.tenderStatus=='done'}">
													<span class="blueBack textstyle">낙찰</span>
												</c:when>
												<c:when
													test="${auction.tenderStatus=='ing' && auction.dday>=0}">
													<span class="grayBack textstyle">미정</span>
												</c:when>
												<c:when test="${auction.tenderStatus=='choo'}">
													<span class="pinkBack textstyle">예약</span>
												</c:when>
											</c:choose></td>
										<td><fmt:formatNumber type='currency'
												value='${auction.tenderBudget}' pattern='###,###' />만원</td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
		...
		<c:if test="${count != 0}">
			<nav>
			<ul class="pagination">
				<li <c:if test="${beginPage==1}">class="disabled"</c:if>><c:choose>
					...
					</c:choose> <span aria-hidden="true">&laquo;</span> </a></li>
				<c:forEach var="i" begin="${beginPage}" end="${endPage}">
					<li <c:if test="${i eq pageNo}">class="active"</c:if>><c:choose>
				...
				<li <c:if test="${lastPage==endPage}">class="disabled"</c:if>>
					<c:choose>
						<c:when test="${lastPage!=endPage}">
							<a href="auctionList.do?pageNo=${pageResult.beginPage + 1}"
								aria-label="Next">
						</c:when>
						<c:otherwise>
							<a href="#" aria-label="Next">
						</c:otherwise>
					</c:choose> <span aria-hidden="true">&raquo;</span> </a>
				</li>
			</ul>
			</nav>
		</c:if>

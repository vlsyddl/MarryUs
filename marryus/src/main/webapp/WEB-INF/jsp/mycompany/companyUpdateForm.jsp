						...
						<div id="card">
							<form id="UpdateProfileForm" method="POST">
								...
								<div class="signup__field">
									<label for="name" class="label">회사 이름</label> <input
										type="text" name="name" id="com_name" class="input-field"
										value="${member.memName}"> <input type="hidden"
										name="email" id="mem_email" class="input-field"
										value="${member.memEmail}"> <input type="hidden"
										name="no" id="no" class="input-field" value="${member.memNo}">
									<input type="hidden" name="comNo" id="com_no"
								...
								<div class="signup__field">
									<label for="password" class="label">새 비밀번호 확인</label> <input
										type="password" name="passCheck" id="password"
										class="input-field">
								</div>

								<div class="signup__field">
									<label for="com_phone" class="label">회사 전화번호</label> <input
										type="text" name="phone" id="com_phone" class="input-field"
										maxlength="13" required value="${member.memPhone}">
								</div>


								<div class="signup__field">
									<label for="com_phone" class="label">회사 주소</label> <input
										type="text" id="sample4_postcode" class="hidden"
										placeholder="우편번호"> <input type="text" name="comAddr"
										id="sample4_roadAddress" placeholder="도로명주소"
										value="${member.comAddr}"> <input type="text"
										id="sample4_jibunAddress" placeholder="지번주소" class="hidden">
									<input type="button" class="btn" onclick="address()"
										value="우편번호 찾기" style="position: relative; left: 0px;">
									<span id="guide" style="color: #999"></span>
								</div>
								<div class="signup__field">
									<label for="com_addr_detail" class="label">회사 상세주소</label> <input
										type="text" name="comAddrDetail" id="comAddrDetail"
										class="input-field" value="${member.comAddrDetail}" required>
								</div>
								...
								<fieldset>
									...
									<div class="signup__field" style="margin-top: 12px;">
										<label class="label" for="comType">대표업종 선택</label> <span
											class="select-field"> <select name="comType"
											id="comType" class="select-field__menu">
												<option>업종선택</option>
												<option value="v"
													${member.comType == 'v' ? 'selected="selected"' : '' }>웨딩홀</option>
												<option value="s"
													${member.comType == 's' ? 'selected="selected"' : '' }>스튜디오</option>
												<option value="d"
													${member.comType == 'd' ? 'selected="selected"' : '' }>드레스</option>
												<option value="m"
													${member.comType == 'm' ? 'selected="selected"' : '' }>메이크업</option>
												<option value="h"
													${member.comType == 'h' ? 'selected="selected"' : '' }>허니문</option>
												<option value="j"
													${member.comType == 'j' ? 'selected="selected"' : '' }>예물</option>
												<option value="e"
													${member.comType == 'e' ? 'selected="selected"' : '' }>기타</option>
										</select>
										</span>
									</div>

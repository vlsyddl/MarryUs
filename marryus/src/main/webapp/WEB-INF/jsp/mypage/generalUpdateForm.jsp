						...
						<div id="card">
							<form id="UpdateProfileForm" method="post"
								...
								<div class="imgs_wrap">
									<c:choose>
										<c:when
											test="${not empty member.genProfilepath && not empty member.genProfilename}">

											<img
												src="<c:url value="/${member.genProfilepath}/${member.genProfilename}" />"
												onclick="fileUploadAction();" id="genProfile"
												class="img-circle img-responsive center-block profile"
												style="width: 150px; height: 150px; object-fit: cover; object-position: top;"
												style="margin: 30px;" />
										</c:when>
										<c:otherwise>
											<img
												src="https://images.unsplash.com/photo-1470320691330-ae8e9288fb77?ixlib=rb-0.3.5&ixid=eyJhcHBfaWQiOjEyMDd9&s=2eb2635304319feaaa15b104729138e4&auto=format&fit=crop&w=1050&q=80"
												onclick="fileUploadAction();" id="genProfile"
								...
								<div class="signup__field">
									<label for="last_name" class="label">이름</label> <input
										type="text" name="name" id="name" class="input-field"
										value="${member.memName}" placeholder="이름을 입력해주세요."> <input
										type="hidden" name="email" id="mem_email" class="input-field"
										value="${member.memEmail}"> <input type="hidden"
										name="no" id="no" class="input-field" value="${member.memNo}">
									<input type="hidden" name="genNo" id="gen_no"
										class="input-field" value="${member.memNo}">
								</div>

								<div class="signup__field">
									<label for="password" class="label">비밀번호</label> <input
										type="password" name="prePass" id="password"
										class="input-field" placeholder="4자 이상 16자 이하" required>
								</div>


								<div class="signup__field">
									<label for="password" class="label">새비밀번호</label> <input
										type="password" name="pass" id="password" class="input-field"
										placeholder="4자 이상 16자 이하" required>
								</div>

								<div class="signup__field">
									<label for="password" class="label">새비밀번호 확인</label> <input
										type="password" name="rePass" id="rePassword"
										class="input-field" placeholder="한번더 입력해주세요." required>
								</div>

								<div class="signup__field">
									<label for="last_name" class="label">핸드폰 번호</label> <input
										type="text" name="phone" id="gen_phone" class="input-field"
										value="${member.memPhone}" iplaceholder="ex)01012346087"
										required>
								</div>
								<div class="signup__field">
									<label for="last_name" class="label">생년월일</label> <input
										type="text" name="genBirth" id="gen_birth" class="input-field"
										value="${member.genBirth}" placeholder="ex)19900619" required>
								</div>
								<input type="hidden" value="mg" name="type" /> <a
									href="<c:url value="mywedding.do" />" class="cancle btn">취소</a>
								<a href="#" class="save btn" id="savebutton"
									onclick="updateProfile()">등록</a>

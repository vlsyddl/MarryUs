...
<link rel="stylesheet"
	href="<c:url value="/resources/css/wedding.css"/>">
<script type="text/javascript"
	src="//dapi.kakao.com/v2/maps/sdk.js?appkey=dc6291b36d6e91a7fc6b30e92a9171d3"></script>
<script type="text/javascript"
									...
									<c:forEach var="l" items="${like}">
										<div class="col-md-4 itemBox" data-href="${l.comInfoNo}">
											<div class="item">
												<div class="imgBox">
													<a href="#"><img
														src="<c:url value="/${l.comFilePath}/${l.comFileName}"/>"
														alt="" class="img-responsive center-block"
														onError="javascript:this.src='<c:url value="/resources/img/sorry.png"/>'"></a>
												</div>
												<div class="textBox">
													<h5>
														<a href="#">${l.comInfoName}</a>
													</h5>
													<p>
														<%-- ${l.comInfoType} <br/> --%>
														${l.comInfoAddr} <br /> ${l.comInfoAddrDetail}
													</p>
												</div>
											</div>
										</div>
									...
									<c:if test="${count != 0}">
										...
										<ul class="pagination">
											<li <c:if test="${beginPage==1}">class="disabled"</c:if>>
												...
												</c:choose> <span aria-hidden="true">&laquo;</span> </a>
											</li>
											...
											<li <c:if test="${lastPage==endPage}">class="disabled"</c:if>>
												<c:choose>
													<c:when test="${lastPage!=endPage}">
														<a
															href="auctionList.do?pageNo=${pageResult.beginPage + 1}"
															aria-label="Next">
													</c:when>
													<c:otherwise>
